...
export const PedidosTable = ({ pedidos, onReorder }: PedidosTableProps) => {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    // Lista REAL com base em prioridade ASC — esta é a que vale!
    const pedidosPorPrioridade = [...pedidos]
      .filter((p) => p.status === 'AGUARDANDO')
      .sort((a, b) => (a.prioridade ?? 9999) - (b.prioridade ?? 9999));
  
    const activeIndex = pedidosPorPrioridade.findIndex((p) => p.id === active.id);
    const overIndex = pedidosPorPrioridade.findIndex((p) => p.id === over.id);
  
    if (activeIndex === -1 || overIndex === -1) return;
  
    // Faz o reordenamento na lista real (independente da ordenação da UI)
    const [pedidoMovido] = pedidosPorPrioridade.splice(activeIndex, 1);
    pedidosPorPrioridade.splice(overIndex, 0, pedidoMovido);
  
    // Atualiza todas as prioridades de forma coerente
    pedidosPorPrioridade.forEach((pedido, index) => {
  ...
  <tr>
    <th></th>
    <SortableHeader sortKey="prioridade" currentSort={sortKey} sortDirection={sortDirection} onSort={handleSort} className="text-left">
      #
    </SortableHeader>
    <SortableHeader sortKey="numeroPedido" currentSort={sortKey} sortDirection={sortDirection} onSort={handleSort} className="text-left">
      Pedido
    </SortableHeader>
    <SortableHeader sortKey="data" currentSort={sortKey} sortDirection={sortDirection} onSort={handleSort} className="text-center">
      DT. Pedido
    </SortableHeader>
    <SortableHeader sortKey="dataConfirmacao" currentSort={sortKey} sortDirection={sortDirection} onSort={handleSort} className="text-center">
      DT. Confirmação
    </SortableHeader>
    ...
    >
      ...
      <div className="space-y-2">
        <div className="grid grid-cols-1 gap-2 text-sm">
          ...
          <div className="flex justify-between">
            <span className="text-muted-foreground">DT. Confirmação:</span>
            <span className="font-medium">{new Date(pedido.dataConfirmacao).toLocaleDateString('pt-BR')}</span>
          </div>
