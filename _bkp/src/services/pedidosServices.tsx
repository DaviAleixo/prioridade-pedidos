...
export const pedidosService = {
  async buscarPedidoSupraWS(): Promise<Pedido[]> {
    ...
    try {
      ...
      // const pedidosJSON = await pedidosService.atualizarPedidoJson(pedidosWS);
      // console.log('Pedidos carregados: atualizarPedidoJson');
      // console.log(pedidosJSON);




      const pedidosConvertidos: Pedido[] = pedidosWS.map((pedido: any) => {
      ...
      console.log(pedidosComPrioridade)
    ...
    try {
      ...
      const pedidosConvertidos: Pedido[] = pedidosApi.map((pedido: any) => {
        ...
        return {
          ...
          vendedor: pedido.nomeVendedor ?? '---',
          data: formatarData(pedido.dataEmissao),
          dataConfirmacao: formatarData(pedido.dataEmissao),
          dataEntrega: '',
          itens: pedido.quantidadeVolumes ?? 0,
          siglaEstado: pedido.siglaEstado ?? '---',
          prioridade: null,
...
function formatarData(dataStr: string): string {
  //const [data] = dataStr.replace(',', '').split(' ');
